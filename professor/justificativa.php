...
function setFormJustificativa($conn, $curso, $data_envio, $tipo_falta, $motivo, $nomeArquivo)
{
    $stmt = $conn->prepare("INSERT INTO tb_formsJustificativa (id_usuario, id_curso, data_envio, id_tipo_falta,id_motivo, nome_arquivo, status, observacoes_coordenador) VALUES (?, ?, ?, ?, ?, ?, 'PENDENTE', NULL)");
    $stmt->execute([$_SESSION['id_usuario'], $curso, $data_envio, $tipo_falta ,$motivo, $nomeArquivo]);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    setFormJustificativa($conn, $curso, $data_envio, $tipo ,$motivo,  $nomeArquivo);
                        ...
                        <div class="mb-3">
                            ...
                            <select style="margin-bottom: 5%;" class="form-select" name="tipo_falta" id="motivo" onchange="exibDiv()" required>
                                ...
                                <?php foreach($tipos_falta as $tipo_falta): ?>
                                    <option value="<?= $tipo_falta['id_tipo_falta'] ?>"><?= $tipo_falta['tipo'] ?></option>
                                <?php endforeach ?>
                            </select>
                            ...
                            <div id="faltaMedica" class="motivoDiv" hidden>
                                ...
                                <select class="form-select" name="selectFaltaMedica" id="selectFaltaMedica">
                                    ...
                                    <?php foreach($faltas_medica as $falta_medica): ?>
                                        <option value="<?= $falta_medica['id_motivo'] ?>"><?= $falta_medica['motivo'] ?></option>
                                    <?php endforeach?>
                                </select>
                            </div>
                            ...
                            <div id="faltaLT" class="motivoDiv" hidden>
                                ...
                                <select class="form-select" name="selectFaltaLT" id="selectFaltaLT">
                                    ...
                                    <?php foreach($faltas_legislacao as $falta_legislacao):?>
                                        <option value="<?= $falta_legislacao['id_motivo'] ?>"><?= $falta_legislacao['motivo'] ?></option>
                                    <?php endforeach ?>
